...
|------|------|
| **智能缓存** | 同一个转发消息的摘要会被缓存 |
| **避免重复调用** | 再次引用相同转发时直接使用缓存，节省API调用 |
| **可配置过期时间** | 默认1小时，可自定义TTL |
| **自动清理** | 过期缓存自动清理，不占用内存 |

### 5. 🖼️ 多模态摘要支持 (v1.2.0新增!)

| 特性 | 说明 |
|------|------|
| **图片传入** | 摘要时可将转发中的图片传给多模态模型 |
| **智能理解** | LLM可结合图片内容生成更准确的摘要 |
| **数量限制** | 可配置最多传入几张图片，控制token消耗 |
...
| summary_cache_ttl | int | 3600 | 摘要缓存过期时间(秒) |
| summary_include_images | bool | false | 摘要时传图片给多模态模型 |
| summary_max_images | int | 5 | 摘要时最多传几张图片 |
...
### v1.2.1
- 🐛 修复AI无法理解注入内容的问题 - 添加上下文说明前缀
- 🐛 修复多模态摘要时图片传递失败的问题 - 直接传URL由provider处理
- 📝 注入内容现在会添加明确说明：`【以下是用户引用/分享的转发消息内容】`

### v1.2.0
- ✨ 新增多模态摘要支持，可将图片传给模型
- 🔧 修复 `image_mode` 属性未定义的bug
- ⚙️ 新增 `summary_include_images` 配置项
- ⚙️ 新增 `summary_max_images` 配置项

### v1.1.0
- ✨ 新增摘要缓存功能，避免重复分析同一转发消息
- 🔧 修复供应商ID缓存问题，配置修改后实时生效
- 📝 添加详细调试日志，显示缓存命中情况
- ⚙️ 新增 `summary_cache_ttl` 配置项
